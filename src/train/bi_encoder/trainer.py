...
from src.entity.config_entity import BiEncoderTrainingConfig
from src.models.bi_encoder.model import BiEncoder
from src.common.logging_config import get_logger
from src.models.bi_encoder.loss import MultipleNegativesRankingLoss, CachedMultipleNegativesRankingLoss
from src.train.bi_encoder.dataset import BiEncoderDataset, create_dataloader
...
class BiEncoderTrainer:
    """Bi-Encoder training with pure PyTorch"""

    def __init__(self, config: BiEncoderTrainingConfig):
        self.config = config
        ...
        self.all_saved_files: Set[Path] = set()  # Track everything for S3 upload
    ...
    def prepare_data(self):
        ...
        train_dataset = BiEncoderDataset(
            ...
            max_length=safe_int(self.config.max_seq_length)
        )
        ...
        self.train_loader = create_dataloader(
            ...
            shuffle=True,
            num_workers=4  # Changed from 4 for stability
        )
    ...
    def build_model(self):
        ...
        # Loss function
        if self.config.loss_type == "CachedMultipleNegativesRankingLoss":
            self.criterion = CachedMultipleNegativesRankingLoss(
                scale=self.config.loss_params.get('scale', 20.0),
                mini_batch_size=self.config.batch_size,
                cache_size=self.config.loss_params.get('cache_size', 1024)
            )
        else:
    ...
    def train_epoch(self, epoch: int):
        ...
        for batch_idx, (queries, positives) in enumerate(pbar):
            # Move to device
            queries = {k: v.to(self.device) for k, v in queries.items()}
            positives = {k: v.to(self.device) for k, v in positives.items()}
            ...
            if self.config.use_fp16:
                with torch.amp.autocast(device_type='cuda'):
                    ...
                    positives_embeddings = self.model.encode(**positives)
                    loss = self.criterion(query_embeddings, positives_embeddings)
    ...
    def validate(self):
        ...
        for queries, positives in tqdm(self.val_loader, desc='Validation'):
            # Move to device
            queries = {k: v.to(self.device) for k, v in queries.items()}
            positives = {k: v.to(self.device) for k, v in positives.items()}
            ...
            loss = self.criterion(query_embeddings, positive_embeddings)
    ...
    def _upload_checkpoint_to_s3(self, checkpoint_path: Path, run_id: str):
        ...
        # S3 path: s3://bucket/models/bi-encoder/{run_id}/checkpoint_epoch_N.pt
    ...
    def register_model_to_registry(self, run_id: str):
        ...
        try:
            model_version = registry.register_model(
                ...
                tags={
                    'framework': 'pytorch',
                    'task': 'bi-encoder-legal-retrieval',
                ...
                description=f"Bi-Encoder for legal document retrieval. "
